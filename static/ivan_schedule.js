 ...
 *
 * Features:
 *  - Multi-week stacked view (4 weeks)
 *  - Double-click any card to edit
 *  - All fields visible on card — no click-to-expand
 *  - Full green fill on completed cards
 *  - Driver name + sequence at top of every card
 *  - Sequence optional/auto-assigned, not in forms
 *  - No start location (removed from UI)
 *  - PU/DE appt status per leg
 *  - Pickup/delivery location names
 *  - "Add Delivery" defaults to next business day
 *  - Missing-delivery amber indicator + quick Add Delivery
 *  - Driver dropdown from Ivan Drivers localStorage + Unassigned
 *  - ZIP → City/State auto-fill (zippopotam.us)
 *  - 24-hour military time throughout
 *  - Audit log sidebar with revert support
 */
var IvanScheduleApp = (function () {
    ...
    /** Next business day (Mon-Fri) after isoDate */
    ...
    /** Relative time string for audit log */
    ...
    // ── Deadhead (always from BASE — start location removed) ─────────────────
    ...
    function _computeMissingDel() {
        var all=_allAsgns(), hasPickup={}, hasDelivery={};
        all.forEach(function(a) {
            if (!a.loadId) return;
            if (a.actionType==='PICKUP') hasPickup[a.loadId]=a;
            if (a.actionType==='DELIVERY'||a.actionType==='PICKUP_AND_DELIVER') hasDelivery[a.loadId]=true;
        });
        _missingDel={};
        Object.keys(hasPickup).forEach(function(lid){ if(!hasDelivery[lid]) _missingDel[lid]=true; });
    }
    ...
    // ── Card — all fields visible, driver+seq at top ──────────────────────────
    function _cardHTML(a, dhMi, dc) {
        ...
        var driverName=(a.driverName||'').trim()||'Unassigned';
        var seq=a.sequenceNumber;
        ...
        // ── Line 1: Driver name + seq | actions ──
        h+='<div class="sc-cv-hdr">';
        h+='<div class="sc-cv-drv">';
        h+='<span class="sc-cv-drv-name">'+_e(driverName)+'</span>';
        if (seq&&seq>0) h+=' <span class="sc-cv-seq">\u00b7 '+seq+'</span>';
        h+='</div>';
        h+='<div class="sc-card-acts">';
        h+='<button class="sc-btn-icon" data-action="toggle-edit" data-id="'+a.id+'" title="Edit (or double-click card)">\u270e</button>';
        h+='<button class="sc-btn-icon sc-btn-del" data-action="del-asgn" data-id="'+a.id+'" title="Delete">\u00d7</button>';
        h+='</div></div>';

        // ── Line 2: Action badge | PRO# chip | DH ──
        ...
        if (tmsId||puNum) {
            ...
            h+='<div class="sc-load-ids">'+_e(ids.join(' \u00b7 '))+'</div>';
        }

        // ── Line 4-5: Locations + cities ──
        if (puLoc||orig) {
        ...
        // ── Line 6: Appt times + per-leg status ──
    ...
    // ── Edit form — no seq, no start location ────────────────────────────────
    ...
    // ── Delivery creation form — prefilled, next biz day default ─────────────
    ...
    // ── Add form — no seq, no start location ─────────────────────────────────
    ...
    // ── Event binding ─────────────────────────────────────────────────────────
    function _bind(el) {
        // Click — handle data-action buttons
    ...
    function _saveEdit(id) {
        ...
        var asgnBody={
            date:a.date, weekStart:_mondayOf(new Date(a.date+'T00:00:00')),
            driverName:    g('driver'),
    ...
    function _saveNew(day) {
        ...
        var asgnBody={
            ...
            driverName:    driver,
            sequenceNumber:_nextSeq(day, driver),   // auto-assign
    ...
    function _saveDelivery(id) {
        ...
        var asgnBody={
            ...
            puAppt:'', deAppt:g('del-deappt'),
            puApptStatus:'NEED', deApptStatus:g('del-deApptStatus')||'NEED',
            notes:g('del-notes'),
        };
        _api('POST','/api/ivan/schedule/assignments',asgnBody)
            .then(function(){ _reload(); })
            .catch(function(err){ alert('Save failed: '+err.message); if(saveBtn){saveBtn.disabled=false;saveBtn.textContent='\u26a1 Save Delivery';} });
    }
